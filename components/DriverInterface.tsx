...
export function DriverInterface() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (typeof data.bus.gpsActive === "boolean") {
          setGpsActive(data.bus.gpsActive);
        }
        setLoadState("ready");
      } catch {
        if (!cancelled) {
  ...
  const handleGpsToggle = async () => {
    ...
    try {
      ...
      updateBusFromFeed(controlledBus.id, { gpsActive: next });
    } catch {
      setGpsActive(!next);
      pushNotification("Could not update GPS status on the server.", "error");
    }
  };
    ...
    <div className="space-y-4">
      ...
      {gpsActive && controlledBus && (
        <section className="rounded-3xl border border-amber-400/20 bg-white/5 p-4 shadow-lg backdrop-blur md:p-6">
          ...
          <LiveMap buses={[controlledBus]} useDeviceGps />
        </section>
      )}
      <div className="grid gap-4 lg:grid-cols-[1.5fr_1fr]">
